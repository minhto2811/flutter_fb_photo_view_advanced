...
import 'package:flutter_fb_photo_view_advanced/src/photo_viewer.dart';
import 'package:flutter_fb_photo_view_advanced/src/utils.dart';
...
class FBPhotoView extends StatefulWidget {
  ...
  static displayImage(BuildContext context, List<String> dataSource,
      {File? fileImage,
      int displayIndex = 0,
...
class _FBPhotoViewState extends State<FBPhotoView> {
      ...
      child: Row(
        children: [
          Expanded(
              ...
              child: imageTile(dataSource[0], imageWidth: double.infinity)),
          if (assetCount > 1) ...[
            const Gap(4.0),
            Expanded(
              flex: 1,
              child: Column(
                children: [
                  Expanded(
                    child:
                        imageTile(dataSource[1], imageWidth: double.infinity),
                  ),
        ...
        children: [
          Expanded(
              ...
              child: Column(
                children: [
                  Expanded(
                      child: imageTile(dataSource[0],
                          imageWidth: double.infinity)),
                  if (assetCount > 1) const Gap(4.0),
                  if (assetCount > 1)
                    Expanded(
                        child: imageTile(dataSource[1],
                            imageWidth: double.infinity)),
                ],
              )),
          if (assetCount > 2) ...[
  ...
  Widget carousel() {
    ...
    return Container(
      ...
      child: CarouselSlider(
        ...
        items: dataSource
            .map((assetSource) => imageTile(assetSource, usingCarousel: true))
            .toList(),
      ),
    );
  }
  ...
  Widget imageTileAtIndex(int index, {double? imageWidth}) {
    return imageTile(
      dataSource[index],
      thumbnailSource: ((widget.thumbnailSource?.length ?? 0) > index)
          ? widget.thumbnailSource![index]
          : null,
      index: index,
    ...
    return GestureDetector(
      ...
      child: usingCarousel
          ? Container(
              ...
              child: renderImage(assetSource,
                  fit: fit, imageWidth: imageWidth, imageHeight: imageHeight),
            )
          : renderImage(assetSource,
              fit: fit, imageWidth: imageWidth, imageHeight: imageHeight),
    );
  }
  ...
  Widget renderImage(
    ...
    final type = assetSource.getType;
    return Hero(
      tag: assetSource,
      child: type == ImageType.network
          ...
          : type == ImageType.asset
              ? Image.asset(assetSource,
                  fit: fit, width: imageWidth, height: imageHeight)
              : Image.file(File(assetSource),
                  fit: fit, width: imageWidth, height: imageHeight),
    );
  }
}
